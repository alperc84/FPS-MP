...
remotesync func fire():
	if can_fire:
		...
		for i in bullets:
			...
			if not result.empty():
				if result.collider is BaseCharacter:
					...
					if result.shape == 1:
						result.collider.get_node("suzanne/mouth").visible = false
						result.collider.get_node("col_mouth").disabled = true
					if result.shape == 2:
						result.collider.get_node("suzanne/ear_L").visible = false
						result.collider.get_node("col_ear_L").disabled = true
					if result.shape == 3:
						result.collider.get_node("suzanne/ear_R").visible = false
						result.collider.get_node("col_ear_R").disabled = true
					if result.shape == 4:
						result.collider.get_node("suzanne/eye_L").visible = false
						result.collider.get_node("col_eye_L").disabled = true
					if result.shape == 5:
						result.collider.get_node("suzanne/eye_R").visible = false
		...
		# Camera shake
		shooter.camera.shake(0.008, 0.1)
...
remotesync func pick():
	if shooter != null:
		if shooter is BaseCharacter and !shooter.is_dead and is_pickable:
			if shooter.equipped_weapon == null:
				is_pickable = false
				var weapon_container = shooter.get_node("head/holder/weapon")
